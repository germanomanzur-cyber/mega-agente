...
const openai = new OpenAI({ baseURL: "https://openrouter.ai/api/v1", apiKey: process.env.OPENROUTER_API_KEY || process.env.OPENAI_API_KEY });
// ─── Knowledge base ───────────────────────────────────────────────────────────
...
"hola!", "buenas tardes", "buenos dias", "buenos días", "buen dia", "buen día",
]);

function esMenuTrigger(texto) {
const t = (texto || "").trim().toLowerCase().replace(/[¡!¿?.,]/g, "").trim();
return MENU_TRIGGERS.has(t);
}

// Enruta la opción elegida en el menú interactivo (ids "opt_*").
function handleMenuOption(optId, session, phone) {
const p = session.profile;
switch (optId) {
case "opt_comprar":
session.tier = "tibio";
if (!p.tipo) p.tipo = "compra";
saveLead({ phone, ...p, tier: "tibio", lastMessage: "[menú] comprar/invertir" });
return "¡Buenísimo! Te ayudo a encontrar tu propiedad. ¿En qué zona de Santa Fe estás buscando y es para comprar o invertir?";
case "opt_vender":
session.tier = "tibio";
p.tipo = "venta";
saveLead({ phone, ...p, tier: "tibio", lastMessage: "[menú] vender/tasar" });
return "Perfecto, te ayudo a vender. Para una tasación orientativa (estimación, no vinculante) contame: ¿dónde está la propiedad, qué tipo es (casa, depto, terreno) y cuántos m² tiene?";
case "opt_staging":
session.tier = "tibio";
saveLead({ phone, ...p, tier: "tibio", lastMessage: "[menú] home staging IA" });
return "Con Home Staging IA convertimos las fotos de una propiedad vacía o antigua en una versión moderna y atractiva, para que el comprador vea el potencial antes de descartarla. Mandale las fotos a Germán y te arma la propuesta: https://wa.me/5493424287842";
case "opt_docs":
session.tier = "tibio";
saveLead({ phone, ...p, tier: "tibio", lastMessage: "[menú] documentación" });
return "Te ayudamos a revisar contratos, detectar datos faltantes y ordenar la documentación de tu operación, para que avance más rápido y con menos errores. Coordiná la revisión con Germán: https://wa.me/5493424287842";
case "opt_german":
session.handoffSent = true;
session.pendingHandoff = buildLeadSummary(phone, session);
saveLead({ phone, ...p, tier: session.tier === "frio" ? "tibio" : session.tier, lastMessage: "[menú] hablar con Germán" });
return "Te conecto con Germán Manzur ahora mismo 📲 Escribile por WhatsApp: https://wa.me/5493424287842";
default:
return "__MENU__";
}
}
...
// ─ Selección del menú interactivo (ids opt_*)
if (userText.startsWith("opt_")) {
session.isFirstMessage = false;
return handleMenuOption(userText, session, phoneNumber);
}
