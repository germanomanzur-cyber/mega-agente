...
if(message.type==="audio"){try{const _mid=message.audio.id;const{data:_mi}=await axios.get(`https://graph.facebook.com/v21.0/${_mid}`,{headers:{Authorization:`Bearer ${WHATSAPP_ACCESS_TOKEN}`}});const _ar=await axios.get(_mi.url,{responseType:"arraybuffer",headers:{Authorization:`Bearer ${WHATSAPP_ACCESS_TOKEN}`}});const _fd=new FormData();_fd.append("file",new Blob([_ar.data],{type:"audio/ogg"}),"audio.ogg");_fd.append("model","whisper-large-v3");_fd.append("language","es");const _w=await axios.post("https://api.groq.com/openai/v1/audio/transcriptions",_fd,{headers:{Authorization:`Bearer ${process.env.GROQ_API_KEY}`}});userText=_w.data.text||"__AUDIO__";console.log(`[NICO/WA] TranscripciÃ³n audio: ${userText}`);}catch(_e){console.error("[NICO/WA] Error transcribiendo audio:",_e.message);userText="__AUDIO__";}}
